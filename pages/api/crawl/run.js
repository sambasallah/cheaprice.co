    ...
    if(req.method === 'POST') {
        ...
        .then(async (snaps) => {
          let products = [];
           ...
           await axios.post('https://mp001iwsca.execute-api.eu-west-1.amazonaws.com/dev/crawl', JSON.stringify({products: products}),{headers: {'Content-Type': 'application/json'}})
           .catch((err) => {
               // send email
